<script lang="ts">
	...
	let footerWindows = $derived(
		windows.windows.filter((window) => window.pin || window.match.test($page.url.pathname))
	);
...
<div class="flex flex-row justify-center items-center h-screen w-screen bg-primary text-white">
	<main
	...
	>
		...
		<div class="flex flex-col lg:flex-row items-center justify-between flex-1 px-16 relative">
			<div class="lg:mb-16 lg:w-1/2">
				<h1
					class="text-6xl font-bold whitespace-pre-wrap border-r-8 w-fit border-accent overflow-hidden pr-1"
					class:type-in={$blinking}
					use:typeIn={{ interval: 200, blinking }}
				>
					{$t('common.home.title')}
				</h1>
				<p class="text-2xl">
		...
		<Footer footerItems={footerWindows} />
