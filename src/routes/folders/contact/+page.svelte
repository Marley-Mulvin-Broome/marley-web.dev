<script lang="ts">
  ...
  const handleSend = async () => {
    if (loadState !== "idle" || name.length < 0 || !validator.isEmail(email) || message.length < 0) return;
...
<div class="flex flex-row gap-8 pb-4">
  ...
  >
    ...
    <Input placeholder="Marley Mulvin Broome" validate={(value) => { return value.length > 0 }} invalidMessage={$t('common.contact.invalid-name')} bind:value={name} />
  </div>
  ...
  >
    ...
    <Input placeholder="marley@gmai.com" type="email" validate={(value) => validator.isEmail(value)} invalidMessage={$t('common.contact.invalid-email')} bind:value={email} />
  </div>
...
>
  ...
  <textarea class="bg-primary-light border border-gray-600 rounded-md px-2 py-1 text-sm text-gray-400 focus:outline-none focus:border-accent shadow-inner h-[200px] mb-2" bind:value={message}></textarea>
...
{:else}
  <div class="w-full py-40 flex flex-col items-center justify-center">
    ...
    <button
      class="bg-primary-light text-white rounded-md px-4 py-2 mt-4" onclick={() => { loadState = "idle" }}
    >
